 ...
 * Uses atomic rename-write to avoid races.
 */
export function registerUserInFrameworkStore(params: {
  accountId: string;
  userId: string;
}): { changed: boolean } {
  ...
  try {
    ...
    // start fresh on missing/corrupt file
  }

  if (content.allowFrom.includes(trimmedUserId)) return { changed: false };
